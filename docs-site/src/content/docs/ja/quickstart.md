...
:::caution[ポートの変更は 1 か所では終わりません]
`Dockerfile` には `EXPOSE 8080` が書かれており、利用側の URL にも `:8080` が
入っています。`MEADOW_PORT` だけを変えると、**待ち受けだけが移動して、
そこを指している側は 8080 のまま**になります ― コンテナは正常に起動し、
しかし誰も到達できません。公開ポートと利用側のオリジンも同時に変更してください。
:::
